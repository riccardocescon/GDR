...
class control{
	public:
		...
		void position_enemy(char lvl[][100], int& R, int& C, char entity, int position[]);
	protected:
...
void control::position_enemy (char lvl[][100], int& R, int& C, char entity, int position[]){
	int pr=0;
	int pc=0;
	for (int r=0; r<R; r++){
		for (int c=0; c<C; c++){
...
void rand_nem(int dx, int sx, int su, int giu, int &nemR, int& nemC, char arr[][100], char w){
	...
	do{
		...
		if(su!=rimuovi){
	        if(x>(dx+sx)&&x<=(dx+sx+su)){
	    		nemR--;
	    		rimuovi=  su;
	        }
		}
	    if(giu!=rimuovi){
	        if(x>(dx+sx+su)&&x<=(dx+sx+su+giu)){
	    		nemR++;
	    		rimuovi=  giu;
	        }
		}
	}while(wall_enemy(arr, nemR, nemC, w)==true);
}

void choose_where(int pR, int pC, int& position_mobR,  int& position_mobC, char arr[][100], char w, char mob, char pg){
	int x=pR-position_mobR;
	int y=pC-position_mobC;
	int rim=0;
	if (arr[position_mobR+1][position_mobC] == pg || arr[position_mobR-1][position_mobC] == pg || arr[position_mobR][position_mobC+1] == pg || arr[position_mobR][position_mobC-1] == pg){
		...
		if (y==0 && x>0){
			rand_nem(85, 4, 6, 5, position_mobR, position_mobC, arr, w);
		}else if (y==0 && x<0){
			rand_nem(5, 85, 4, 6, position_mobR, position_mobC, arr, w);
		}else if (x==0 && y>0){
			rand_nem(5, 6, 4, 85, position_mobR, position_mobC, arr, w);
		}else if (x==0 && y<0){
			rand_nem(6, 4, 85, 5, position_mobR, position_mobC, arr, w);
		}else if (x>0 && y>0){
			rand_nem(44, 4, 6, 46, position_mobR, position_mobC, arr, w);
		}else if (x>0 && y<0){
			rand_nem(44, 6, 46, 4, position_mobR, position_mobC, arr, w);
		}else if (x<0 && y>0){
			rand_nem(6, 44, 4, 46, position_mobR, position_mobC, arr, w);
		}else if (x<0 && y<0){
			rand_nem(4, 44, 46, 6, position_mobR, position_mobC, arr, w);
		}
...
void mov_enemy(char lvl[][100], int mobs[], int pR, int pC, int entitys, char wall, char pg){
	...
	int c = 1;
	while(entitys!=0){
		int positionmob_r = mobs[r];
		int positionmob_c = mobs[c];
...
void test(char button, player pg, int map_rows, int map_cols, char lvl[][100]){
...
int main(int argc, char** argv) {
	...
	system("cls");
	bool hit = false;
	int position_enemys[50];
	int entitys = howmany(lvl,  map_level.rows, map_level.cols, map_level.e, map_level.rows, map_level.cols);
	control_enemy.position_enemy(lvl, map_level.rows, map_level.cols, map_level.e, position_enemys);
	...
	do{
		...
		test(control_you.button, pg, map_level.rows, map_level.cols, lvl);
		...
		mov_enemy(lvl, position_enemys, control_you.pR, control_you.pC, entitys, map_level.w, map_level.y);
		if (pg.getLife()<=0){
			break;
		}
	}while(3<4);
